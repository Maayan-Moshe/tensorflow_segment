...
def train_model(session, graph_elements, hyperparams, train_dataset, train_labels, \
    ...
    if 'num_full_epochs' in hyperparams:
        ...
        num_steps = num_full_epochs * num_steps_per_epoch
    else:
        num_steps = hyperparams['num_steps'] # number of runs - minimization iterations
        num_full_epochs = num_steps / float(num_steps_per_epoch)
    
       
    log_step = num_steps // hyperparams['num_logs']
    print('log_step', log_step)
    print("\nRunning total %d steps %1.2f epochs" % (num_steps, num_full_epochs))
    ...
    for step in range(num_steps):
        ...
        if math.isnan(train_loss):
            print('\n !!! Breaking: Model divergence - train Loss is nan on step', step)
            break
